...
class TestLogsToolTypeHints(unittest.TestCase):
    ...
    def test_08_all_exports_are_accessible(self) -> None:
        ...
        for export_name in all_exports:
            self.assertTrue(
                hasattr(logstool, export_name), f"Export '{export_name}' not accessible"
            )
    ...
    def test_09_docstrings_are_preserved(self) -> None:
        ...
        self.assertIn("user-facing client", LoggerClient.__doc__)

        self.assertIsNotNone(LoggerEngine.__doc__)
        self.assertIn("Coordinate engines", LoggerEngine.__doc__)

        # Check method docstrings
        self.assertIsNotNone(LoggerClient.message.__doc__)
        self.assertIn("emit a log message", LoggerClient.message.__doc__.lower())
